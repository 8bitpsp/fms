...
#if defined(UNIX) || defined(S60) || defined(PSP)
#include <unistd.h>
#endif
...
int StartMSX(int NewMode,int NewRAMPages,int NewVRAMPages)
{
...
#ifdef MINIZIP
  if(Kanji=LoadROM("SYSTEM.ZIP/KANJI.ROM",0x20000,0))
...
#ifdef MINIZIP
  if(P=LoadROM("SYSTEM.ZIP/RS232.ROM",0x4000,0))
  {
  ...
  {
#ifdef MINIZIP
    if(LoadCart("SYSTEM.ZIP/MSXDOS2.ROM",2,MAP_GEN16))
...
#ifdef MINIZIP
    if((J<MAXSLOTS)&&LoadCart("SYSTEM.ZIP/PAINTER.ROM",J,0)) ++J;
    else
#endif
  ...
  for(;(J<MAXSLOTS)&&ROMData[J];++J);
#ifdef MINIZIP
  if((J<MAXSLOTS)&&LoadCart("SYSTEM.ZIP/FMPAC.ROM",J,MAP_FMPAC)) ++J;
  else
#endif
  ...
  {
#ifdef MINIZIP
    if(LoadCart("GMASTER2.ZIP/GMASTER2.ROM",J,MAP_GMASTER2)) ++J;
    else if(LoadCart("GMASTER.ZIP/GMASTER.ROM",J,0)) ++J;
    else
#endif
...
int ResetMSX(int NewMode,int NewRAMPages,int NewVRAMPages)
{
...
#ifdef MINIZIP
        if(P1=LoadROM("SYSTEM.ZIP/MSX.ROM",0x8000,0));
        else
#endif
...
#ifdef MINIZIP
        if(P1=LoadROM("SYSTEM.ZIP/MSX2.ROM",0x8000,0));
        else
#endif
...
#ifdef MINIZIP
        if(P2=LoadROM("SYSTEM.ZIP/MSX2EXT.ROM",0x4000,0));
        else
#endif
...
#ifdef MINIZIP
        if(P1=LoadROM("SYSTEM.ZIP/MSX2P.ROM",0x8000,0));
        else
#endif
...
#ifdef MINIZIP
        if(P2=LoadROM("SYSTEM.ZIP/MSX2PEXT.ROM",0x4000,0));
        else
#endif
...
#ifdef MINIZIP
    if(P1=LoadROM("SYSTEM.ZIP/DISK.ROM",0x4000,0));
    else
#endif
  ...
  /* Reset sound chips */
  Reset8910(&PSG,(1000*CPU_CLOCK)>>1,0);
...
byte ChangeDisk(byte N,const char *FileName)
{
  ...
  if(*FileName&&LoadFDI(&FDD[N],FileName,FMT_AUTO)) return(1);
...
int LoadFile(const char *FileName)
{
  ...
  if(!stricmpn(T,".DSK",4)||!stricmpn(T,".FDI",4))
  {
  ...
  if(!stricmpn(T,".ROM",4)) return(!!LoadCart(FileName,0,ROMGUESS(0)|ROMTYPE(0)));
  /* Try loading as a font */
  if(!stricmpn(T,".FNT",4)) return(!!LoadFNT(FileName));
