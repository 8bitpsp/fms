...
#define fread(B,L,N,F) gzread(F,B,(L)*(N))
  ...
  if(!F)
  {
    ...
    if(!(Pos=strrchr(Name,'/'))) return(0);

    /* Get the <ZIPFILE> portion of Name */
    ZipName=(char*)malloc(sizeof(char)*(Pos-Name+1));
    strncpy(ZipName,Name,Pos-Name);
    ZipName[Pos-Name]='\0';
    ...
    /* Not a ZIP file */
    if(!ZF) { if(!Dsk) free(Dsk1);return(0); }
    ...
    /* Read data */
    if(unzReadCurrentFile(ZF,Dsk1,DSK_DISK_SIZE)!=DSK_DISK_SIZE)
    { 
      unzClose(ZF);
      if(!Dsk) free(Dsk1); 
      return(0); 
