...
#ifdef MINIZIP
  unzFile ZF;
  /* Attempt to open SYSTEM.ZIP */
  if((ZF=unzOpen("SYSTEM.ZIP"))
    &&(unzLocateFile(ZF,"COLECO.ROM",0)==UNZ_OK)
    &&(unzOpenCurrentFile(ZF)==UNZ_OK))
  {
    /* Read */
    if(unzReadCurrentFile(ZF,ROM_BIOS,0x2000)!=0x2000) P="SHORT FILE";

    /* Done; close archived file */
...
#ifdef MINIZIP
    if(ZF
      &&(unzLocateFile(ZF,"WRITER.ROM",0)==UNZ_OK)
      &&(unzOpenCurrentFile(ZF)==UNZ_OK))
    {
      /* Read */
      if(unzReadCurrentFile(ZF,ROM_WRITER,0x8000)==0x8000) ++AdamROMs;
...
#ifdef MINIZIP
    if(ZF
      &&(unzLocateFile(ZF,"EOS.ROM",0)==UNZ_OK)
      &&(unzOpenCurrentFile(ZF)==UNZ_OK))
    {
      /* Read */
      if(unzReadCurrentFile(ZF,ROM_EOS,0x2000)==0x2000) ++AdamROMs;
...
int LoadROM(const char *Cartridge)
{
  ...
  /* File does not exist - maybe it's an archived file? */
  if(Cartridge&&!F)
  {
    char *ZipName=strdup(Cartridge);
    char *ArchivedName;

    /* Check if Name is in <ZIPFILE>/<ArchiveFile> notation */
    if(!(ArchivedName=strrchr(ZipName,'/'))) { free(ZipName);return(0); }

    /* Split the Zip and Archive parts */
    *ArchivedName='\0';
    ArchivedName++;

    /* Open archive and locate the requested file */
    if (!(ZF=unzOpen(ZipName))
      ||(unzLocateFile(ZF,ArchivedName,0)!=UNZ_OK)
      ||(unzOpenCurrentFile(ZF)!=UNZ_OK))
    { free(ZipName);if(ZF) unzClose(ZF);return(0); }
  ...
  if(ZF)
  {
    if(unzReadCurrentFile(ZF,Buf,2)!=2) 
    { unzCloseCurrentFile(ZF);unzClose(ZF);return(0); }
  }
  else
#endif
  ...
  /* Done with the file */
#ifdef MINIZIP
  if(ZF) { unzCloseCurrentFile(ZF);unzClose(ZF); }
  else
#endif
  fclose(F);
